...
public class MyStorage {
     ...
     */
    static void prod_input(Scanner s){ // 기능번호 1번
     ...
     */
    static void prod_remove(Scanner s){ // 기능번호 2번
        // 제품 등록 취소 로직을 구현합니다. 입력받은 제품명에 해당하는 배열 요소를 EMPTY로 설정합니다.
        System.out.println("[System] 제품 등록 취소를 원하는 제품명을 입력하세요 :");
        String cancelProduct = s.nextLine();

        // 제품이 지워지는지 체크
        boolean isRemove = false;
        for (int i = 0; i < products.length; i++) {
            // i번째 인덱스와 입력값이 같다면 Empty로 바꾼다.
            if (products[i].equals(cancelProduct)) {
                // 제품명 삭제
                products[i] = EMPTY;
                // 수량 초기화
                counts[i] = 0;
        ...
        // 제품명이 인덱스에 없다면
        if (!isRemove) {
            System.out.println("[Error] 해당 제품은 등록되어 있지 않습니다.");
        }
    }
     ...
     * 반환 타입은 void로, 별도의 반환 값이 없습니다.
     */
    public static void prod_amount_add(Scanner s) { // 기능 번호 3
       ...
       try { // 숫자 타입으로 바꾼다. 문자열은 숫자로 못바꿈
           int cnt = Integer.parseInt(quantityStr);
           // 수량이 0보다 작거나 같을때
           if (cnt <= 0) {
               System.out.println("[Error]수량은 0보다 커야합니다.");
               return;
           }
           ...
           System.out.println("[Error] 수량이 아닙니다.");
           System.out.println("-".repeat(30));
       }
    }
     ...
     * 반환 타입은 void로, 별도의 반환 값이 없습니다.
     */
    public static void prod_amount_decrease(Scanner s) { // 기능구현 4번
        ...
        // 제품이 없으면
        if (foundIdx < 0) {
            System.out.println("[Error] 제품이 존재하지 않습니다.");
            System.out.println("-".repeat(30));
            return;
        }
        ...
        try {
            ...
            // 출고할 수량이 재고보다 적거나 같을 때
            if (counts[foundIdx] >= quantity) {
                counts[foundIdx] -= quantity;  // 재고에서 수량을 차감
            ...
            System.out.println("[Error] 수량은 숫자로만 입력해야 합니다");
            System.out.println("-".repeat(30));
        }
    }
